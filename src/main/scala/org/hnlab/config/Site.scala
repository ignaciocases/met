...
object MenuGroups {
  ...
  val CappGroup = LocGroup("metaphor")
...
object Site extends Locs {
  ...
  val search = MenuLoc(Menu.i("Search") / "search" >> TopBarGroup >> RequireLoggedIn)

  // Cappuccino application
  val capp = MenuLoc(Menu.i("Metaphor") / "metaphor"/ "index" >> CappGroup >> RequireLoggedIn)
  val cappDebug = MenuLoc(Menu.i("Metaphor-debug") / "metaphor"/ "index-debug" >> CappGroup >> RequireLoggedIn)  
